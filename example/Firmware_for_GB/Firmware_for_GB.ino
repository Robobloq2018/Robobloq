 ...
 *           2.1.7             2019/07/11         Allen                      新增加4个RGB-LED 显示固件
 *           2.1.7             2019/07/11         Allen                      新增加4个数码管显示固件
 *           2.1.7             2019/07/12         Allen                      新增加温度传感器读取固件
 *********************************************/
    ...
    switch((uint8_t)RX_BUF[ack][4])
       {
        case Hardware://获取版本信息
                     ...
                     TX_BUF[1] = 0X42;
                     TX_BUF[2] = 0X08;
                     TX_BUF[3] = RX_BUF[ack][3];
                     TX_BUF[4] = DeviceType;
                     TX_BUF[5] = HardwareVersion;
                     TX_BUF[6] = 7;
                     for(i=0;i<7;i++)
                         TX_CheckSum += TX_BUF[i];
                     TX_BUF[7] = TX_CheckSum;
                     RB_Serial.Serial_SendString(TX_BUF,8);
        ...
        case Sound_Read:
           ...
           case TouchSensor_Read:
            ...
            case TemptureSensors_Read:
                      ...
                      temp_value = TEMPERATURE->GET_TTEMPERATURE((uint8_t)(RX_BUF[ack][6]));
...
{    
    ...
    uint8_t  randnum;
     ...
     if(Distance>55)  { flag = 0; last_flag = 0; }
     ...
     switch(last_flag) {
             ...
             case 1:
                   ...
                   RGBled.show();
                   randnum = random(200);
                    if(randnum>100){ RB_DcmotorOnBoard.run(-98,98);}
                   else { RB_DcmotorOnBoard.run(98,-98);}
                   delay(550);
                   break;
             case 2: 
          ...
          switch(Work_Mode)
           {
              case Remote_Control_Mode:
                     RGBled.setColor(0,0,50,0);
                     RGBled.show();
                     Work_Mode = Line_Follower_Mode;
                     Work_Mode_Port =2 ;
                      break;
              case Line_Follower_Mode:
                     RGBled.setColor(0,50,0,0);
                     RGBled.show();
                     Work_Mode = Ultrasonic_Mode;
                     Work_Mode_Port =3 ;
                      break;
              case Ultrasonic_Mode:
                     RGBled.setColor(0,0,0,50);
                     RGBled.show();
                     Work_Mode = Remote_Control_Mode;
                      break;
            }  
       }
}
